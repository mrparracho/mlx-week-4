...
class ImageCaptioningModel(nn.Module):
    ...
    Supports both cross-attention and self-attention (concatenation) modes.
    """
    
    def __init__(self, clip_model_name: str = 'openai/clip-vit-base-patch32', decoder_mode: str = 'cross_attention'):
        super().__init__()
        assert decoder_mode in ['cross_attention', 'self_attention'], "decoder_mode must be 'cross_attention' or 'self_attention'"
        ...
        # Custom TransformerEncoder for self-attention mode
        if decoder_mode == 'self_attention':
            # Use GPT-2 config for fair comparison
            self.transformer_encoder = nn.TransformerEncoder(
                nn.TransformerEncoderLayer(
                    d_model=gpt_hidden_size,
                    nhead=config.num_attention_heads,
                    dim_feedforward=config.n_inner if config.n_inner is not None else 4 * gpt_hidden_size,
                    dropout=config.resid_pdrop,
                    activation='gelu',
                    batch_first=True
                ),
                num_layers=config.n_layer,
                norm=nn.LayerNorm(gpt_hidden_size, eps=config.layer_norm_epsilon)
            )
            self.lm_head = nn.Linear(gpt_hidden_size, self.decoder.config.vocab_size, bias=False)
    ...
    def build_concat_attention_mask(self, num_img_tokens, num_caption_tokens, device, batch_size=1):
        """Builds a custom attention mask for concatenated image+caption input, returns float mask for HuggingFace GPT-2."""
        L = num_img_tokens + num_caption_tokens
        mask = torch.zeros((L, L), dtype=torch.bool, device=device)
        # Image tokens: can attend to all
        mask[:num_img_tokens, :] = 1
        # Caption tokens: can attend to all image tokens
        mask[num_img_tokens:, :num_img_tokens] = 1
        # Caption tokens: causal mask for caption part
        causal = torch.tril(torch.ones((num_caption_tokens, num_caption_tokens), dtype=torch.bool, device=device))
        mask[num_img_tokens:, num_img_tokens:] = causal
        # Expand for batch
        mask = mask.unsqueeze(0).expand(batch_size, L, L)  # [batch, L, L]
        # Convert to float mask: 0.0 for attend, -inf for mask
    ...
    def forward(self, images, input_ids, attention_mask=None, labels=None):
        ...
        if self.decoder_mode == 'cross_attention':
            ...
            # Decode with GPT-2 (cross-attention)
            ...
            # Self-attention: concatenate image embeddings to caption embeddings
            device = next(self.parameters()).device
            batch_size = input_ids.size(0)
            # 1. Encode images (B, num_patches, hidden)
            image_embeds = self.encode_images(images)  # (B, num_patches, hidden)
            num_img_tokens = image_embeds.size(1)
            num_caption_tokens = input_ids.size(1)
            # 2. Get caption token embeddings (B, seq_len, hidden)
            caption_embeds = self.decoder.transformer.wte(input_ids)  # (B, seq_len, hidden)
            # 3. Concatenate image and caption embeddings
            concat_embeds = torch.cat([image_embeds, caption_embeds], dim=1)  # (B, num_patches+seq_len, hidden)
            # 4. Build custom attention mask (float, [B, L, L])
            concat_mask = self.build_concat_attention_mask(num_img_tokens, num_caption_tokens, device, batch_size)
            # Debug prints for shape checking
            print(f"[DEBUG] image_embeds.shape: {image_embeds.shape}")
            print(f"[DEBUG] caption_embeds.shape: {caption_embeds.shape}")
            print(f"[DEBUG] concat_embeds.shape: {concat_embeds.shape}")
            print(f"[DEBUG] concat_mask.shape: {concat_mask.shape}")
            # 5. Adjust labels (shift right for language modeling, ignore image tokens)
            if labels is not None:
                # Pad with -100 for image tokens so loss is not computed on them
                ignore_img = torch.full((batch_size, num_img_tokens), -100, dtype=labels.dtype, device=labels.device)
                concat_labels = torch.cat([ignore_img, labels], dim=1)
            else:
                concat_labels = None
            # 6. Positional embeddings
            position_ids = torch.arange(concat_embeds.size(1), dtype=torch.long, device=device).unsqueeze(0).expand(batch_size, -1)
            pos_embeds = self.decoder.transformer.wpe(position_ids)
            hidden_states = concat_embeds + pos_embeds
            hidden_states = self.decoder.transformer.drop(hidden_states)
            print(f"[DEBUG] hidden_states.shape before custom encoder: {hidden_states.shape}")
            # 7. Pass through custom TransformerEncoder
            # nn.TransformerEncoder expects mask shape [B, L, L] as bool or float (float: 0 for attend, -inf for mask)
            # Ours is float: 0.0 for attend, -inf for mask
            output = self.transformer_encoder(hidden_states, mask=concat_mask[0])  # mask shape [L, L], broadcasted for batch
            # 8. LM head
            ...
            if concat_labels is not None:
                ...
                loss = loss_fct(logits.view(-1, logits.size(-1)), concat_labels.view(-1))
    ...
    def generate_caption(self, image, max_length=50, num_beams=5, temperature=0.8):
        ...
        if self.decoder_mode == 'cross_attention':
            ...
            # Self-attention: greedy decoding (no beam search for simplicity)
            prompt = "A photo shows"
            input_ids = self.tokenizer(prompt, return_tensors="pt").input_ids.to(device)
            max_gen_len = max_length
            generated = input_ids
            for _ in range(max_gen_len):
                # Prepare image embeds (repeat for batch if needed)
                if generated.size(0) > 1:
                    image_embeds = self.encode_images([image]*generated.size(0))
                else:
                    image_embeds = self.encode_images(image)
                num_img_tokens = image_embeds.size(1)
                num_caption_tokens = generated.size(1)
                caption_embeds = self.decoder.transformer.wte(generated)
                concat_embeds = torch.cat([image_embeds, caption_embeds], dim=1)
                concat_mask = self.build_concat_attention_mask(num_img_tokens, num_caption_tokens, device, generated.size(0))
                position_ids = torch.arange(concat_embeds.size(1), dtype=torch.long, device=device).unsqueeze(0).expand(generated.size(0), -1)
                hidden_states = concat_embeds + self.decoder.transformer.wpe(position_ids)
                hidden_states = self.decoder.transformer.drop(hidden_states)
                for block in self.decoder.transformer.h:
                    hidden_states = block(hidden_states, attention_mask=concat_mask)[0]
...
def main():
    ...
    parser.add_argument("--decoder-mode", type=str, default="cross_attention", choices=["cross_attention", "self_attention"],
                       help="Decoder mode: 'cross_attention' (default) or 'self_attention' (concatenate image and caption embeddings)")
    ...
    # Get wandb values from environment variables if not provided
    if not args.wandb:
        # Check if wandb is enabled via environment
    ...
    # Ensure wandb_project is always a string
    ...
    # Initialize model
    model = ImageCaptioningModel(decoder_mode=args.decoder_mode)
